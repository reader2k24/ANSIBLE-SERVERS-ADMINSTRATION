...
for interface in $interfaces; do
    ...
    if [[ "$state" == "up" ]]; then
        ...
        if [[ "$interface" == "ens36" ]]; then
            IPV4_ADDRESS="11.11.11.100/24"
            IPV6_ADDRESS="1110:A::100/64"
        elif [[ "$interface" == "ens37" ]]; then
            IPV4_ADDRESS="131.131.0.1/26"
            IPV6_ADDRESS="131:131:d::1/122"
        else
...
ip route add 0.0.0.0/0 via 11.11.11.1 proto static metric 100
ip -6 route add ::/0 via 1110:A::1 proto static metric 100

# Устанавливаем параметры forwarding для IPv4 и IPv6
sysctl -w net.ipv4.ip_forward=1
sysctl -w net.ipv6.conf.all.forwarding=1
sysctl -w net.ipv6.conf.ens37.accept_ra=2
